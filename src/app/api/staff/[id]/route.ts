...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: existing } = await adminSupabase.from('staff').select('school_id').eq('id', id).single();
    if (existing?.school_id !== profile?.school_id) return NextResponse.json({ error: 'Wrong school' }, { status: 403 });

    const { error } = await adminSupabase.from('staff').update(staff).eq('id', id);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error } = await adminSupabase.from('staff')
