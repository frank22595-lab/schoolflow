...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (acceptable_answers !== undefined) update.acceptable_answers = acceptable_answers;
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Soft delete — questions may already be referenced by exam attempts once
    // exams ship, so we keep the row and just hide it from the bank.
